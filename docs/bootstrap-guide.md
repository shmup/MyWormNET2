...
only include games for requested channel. remove expired games (5 min old).
...
this mywormnet2 codebase is a complete reference. key files:
- `irc.d:30` - password definition
- `http.d:82-143` - all http endpoints
- `common.d:43-48` - channel config structure

the `ae` library handles the heavy lifting for irc protocol. for your implementation, either use an irc library or implement the commands in the irc server section above.
